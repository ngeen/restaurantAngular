...
import {
  auth2StrategyOptions, NbAuthRefreshableToken,
  NbAuthResult, NbAuthStrategy, NbAuthStrategyClass, NbAuthToken, NbOAuth2AuthStrategyOptions, NbOAuth2ClientAuthMethod,
  NbOAuth2GrantType,
  NbOAuth2ResponseType
} from "@nebular/auth";
import {NbAuthIllegalTokenError} from "./token";
...
export class NbOAuth2AuthStrategy extends NbAuthStrategy {
  ...
  authenticate(data?: any): Observable<NbAuthResult> {
    ...
    if (this.getOption('token.grantType') === NbOAuth2GrantType.PASSWORD) {
      return this.passwordToken(data.username, data.password)
    } else {
